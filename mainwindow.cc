...
void MainWindow::update_view()
{
    ...
    try
    {
        ...
        std::unordered_set<AreaID> result_areas;
        MainProgram::CmdResultRoute result_route;
        switch (mainprg_.prev_result.first)
        {
        case MainProgram::ResultType::PLACEIDLIST:
            {
                ...
                result_areas.insert(std::get<MainProgram::CmdResultPlaceIDs>(mainprg_.prev_result.second).first);
            }
            break;
        case MainProgram::ResultType::AREAIDLIST:
            {
            auto& prevresult = std::get<MainProgram::CmdResultAreaIDs>(mainprg_.prev_result.second);
            result_areas.insert(prevresult.begin(), prevresult.end());
            }
            break;
        // case MainProgram::ResultType::CYCLE:
        case MainProgram::ResultType::ROUTE:
        case MainProgram::ResultType::WAYS:
            {
                        ...
                        if (ui->places_checkbox->isChecked())
                        {
                            ...
                            if (ui->placenames_checkbox->isChecked())
                            {
                                try
                                {
                                    ...
                                    if (name == NO_NAME)
                                    {
                                        errorset.insert("get_place_name_type() returned error NO_NAME");
                                    }
        ...
        if (ui->areas_checkbox->isChecked())
        {
            try
            {
                ...
                {
                    errorset.insert("GUI error: all_regions() returned error {NO_REGION}");
                }
                ...
                for (auto areaid : areaids)
                {
                    ...
                    if (areaid != NO_AREA)
                    {
                        ...
                        if (coords.size() < 3 || std::find(coords.begin(), coords.end(), NO_COORD) != coords.end())
                        {
...
void MainWindow::scene_selection_change()
{
    ...
    if (!items.empty())
    {
        ...
        {
            auto data = i->data(0);
//            assert(stopid!=NO_ID && "NO_ID as place in graphics view");
