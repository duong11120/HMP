...
private:
    ...
    std::vector<std::pair<std::string, bool>> test_history_;
    ...
    CmdResult cmd_random_way_network(std::ostream& output, MatchIter begin, MatchIter end);
    ...
    void test_place_name_type(Stopwatch& watch);
    void test_place_coord(Stopwatch& watch);
    void test_find_places_name(Stopwatch& watch);
    void test_find_places_type(Stopwatch& watch);
    void test_change_place_name(Stopwatch& watch);
    void test_area_name(Stopwatch& watch);
    void test_subarea_in_areas(Stopwatch& watch);
    void test_all_subareas_in_area(Stopwatch& watch);
    void test_places_closest_to(Stopwatch& watch);
    void test_remove_place(Stopwatch& watch);
    void test_common_area_of_subareas(Stopwatch& watch);
    void test_ways_from(Stopwatch& watch);
    void test_way_coords(Stopwatch& watch);
    void test_remove_way(Stopwatch& watch);
    void test_route_any(Stopwatch& watch);
    void test_route_least_crossroads(Stopwatch& watch);
    void test_route_shortest_distance(Stopwatch& watch);
    void test_route_with_cycle(Stopwatch& watch);
    void test_trim_ways(Stopwatch& watch);
    void test_comment(Stopwatch& watch);

    void add_random_places_areas(unsigned int size, Stopwatch* watchp, Coord min = {1,1}, Coord max = {10000, 10000});
    void add_random_ways(unsigned int n, Stopwatch* watchp);
    void create_random_way_network(std::ostream& output, int extrapoints, int extraways);
    static Distance coorddist(Coord p1, Coord p2);
    ...
    template<PlaceID(Datastructures::*MFUNC)()>
    void NoParPlaceTestCmd(Stopwatch& watch);

    template<std::vector<PlaceID>(Datastructures::*MFUNC)()>
    void NoParPlaceListTestCmd(Stopwatch& watch);
...
public:
    ...
    Stopwatch(bool use_counter = false) : use_counter_(use_counter)
    {
        ...
        if (use_counter_)
        {
            ...
            if (fd_ == -1) {
                std::error_code ec( ENOTSUP, std::system_category() );
                throw std::system_error( ec, "Couldn't open perf events!" );
            }
        }
#endif
        reset();
...
template<PlaceID(Datastructures::*MFUNC)()>
void MainProgram::NoParPlaceTestCmd(Stopwatch& watch)
{
    watch.start();
    (ds_.*MFUNC)();
    watch.stop();
...
template<std::vector<PlaceID>(Datastructures::*MFUNC)()>
void MainProgram::NoParPlaceListTestCmd(Stopwatch& watch)
{
    watch.start();
    (ds_.*MFUNC)();
    watch.stop();
